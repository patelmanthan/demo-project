...
public class login_validation_dao implements login_validation_dao_int {
	...
	public List<model> login_validation(model v,HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			ResultSet rs=st.executeQuery("SELECT demo.*,address.* FROM demo INNER JOIN address  ON demo.user_id=address.user_id WHERE demo.emailid like '"+v.getLogin_emailid()+"%' && demo.password like '"+v.getLogin_password()+"%'");
